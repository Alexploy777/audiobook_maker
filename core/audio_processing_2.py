...
class M4BMerger(QRunnable):
    ...
    def merge_files(self):
        ...
        try:
            ...
            ffmpeg_command = [
                'ffmpeg', '-f', 'concat', '-safe', '0', '-i', temp_file.name,
    ...
    def add_chapters(self):
        """Добавляет главы с использованием MP4Box."""
        ...
        try:
            with tempfile.NamedTemporaryFile(mode='w', suffix=".txt", delete=False) as chapter_file:
                ...
                for i, file in enumerate(self.input_files, start=1):
                    ...
                    chapter_file.write(f"CHAPTER{i:02d}={int(start_time)}\nCHAPTER{i:02d}NAME=Chapter {i}\n")
                    start_time = end_time

            # Добавляем главы в файл с помощью MP4Box
            mp4box_command = ['MP4Box', '-chap', chapter_file.name, '-out', self.output_file]
            subprocess.run(mp4box_command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except subprocess.CalledProcessError as e:
            print(f'Ошибка при добавлении глав с MP4Box: {e}')
        finally:
