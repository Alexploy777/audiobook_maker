...
class M4BMerger(QRunnable):
    ...
    def add_chapters(self):
        """Добавляет главы для каждого файла из input_files в итоговый m4b файл."""
        self.my_signals.label_info_signal.emit('Добавляю главы')
        self.my_signals.progress_bar_signal.emit(50)
        try:
            # Создаем временный метафайл для ffmpeg, содержащий временные метки глав
            with tempfile.NamedTemporaryFile(mode='w', suffix=".txt", delete=False) as chapter_file:
                ...
                for i, file in enumerate(self.input_files, start=1):
                    # Получаем продолжительность текущего файла
                    ffprobe_command = [
                    ...
                    # Записываем метки для главы
                    chapter_file.write(
                        f"[CHAPTER]\nTIMEBASE=1/1\nSTART={int(start_time)}\nEND={int(end_time)}\ntitle=Chapter {i}\n")
                    start_time = end_time  # Начало следующей главы

            # Добавляем главы в файл с помощью ffmpeg
            ffmpeg_chapter_command = [
                'ffmpeg', '-i', self.output_file, '-i', chapter_file.name,
                '-map_metadata', '1', '-codec', 'copy', '-y', self.output_file
            ]
            subprocess.run(ffmpeg_chapter_command, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except subprocess.CalledProcessError as e:
            print(f'Ошибка при добавлении глав: {e}')
        finally:
            os.remove(chapter_file.name)  # Удаляем временный файл
