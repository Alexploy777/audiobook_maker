# -*- coding: utf-8 -*-

# Form implementation generated from reading ui file 'gui.ui'
#
# Created by: PyQt5 UI code generator 5.15.9
#
# WARNING: Any manual changes made to this file will be lost when pyuic5 is
# run again.  Do not edit this file unless you know what you are doing.


from PyQt5 import QtCore, QtGui, QtWidgets


class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("MainWindow")
        MainWindow.resize(584, 793)
        self.centralwidget = QtWidgets.QWidget(MainWindow)
        self.centralwidget.setObjectName("centralwidget")
        self.verticalLayout_12 = QtWidgets.QVBoxLayout(self.centralwidget)
        self.verticalLayout_12.setObjectName("verticalLayout_12")
        self.gridLayout_2 = QtWidgets.QGridLayout()
        self.gridLayout_2.setObjectName("gridLayout_2")
        self.ControlPanel = QtWidgets.QGroupBox(self.centralwidget)
        self.ControlPanel.setMinimumSize(QtCore.QSize(0, 100))
        self.ControlPanel.setMaximumSize(QtCore.QSize(16777215, 100))
        self.ControlPanel.setObjectName("ControlPanel")
        self.gridLayout_4 = QtWidgets.QGridLayout(self.ControlPanel)
        self.gridLayout_4.setObjectName("gridLayout_4")
        self.progressBar = QtWidgets.QProgressBar(self.ControlPanel)
        self.progressBar.setProperty("value", 0)
        self.progressBar.setTextVisible(False)
        self.progressBar.setObjectName("progressBar")
        self.gridLayout_4.addWidget(self.progressBar, 2, 0, 1, 2)
        self.lcdNumber = QtWidgets.QLCDNumber(self.ControlPanel)
        self.lcdNumber.setMinimumSize(QtCore.QSize(180, 67))
        self.lcdNumber.setSmallDecimalPoint(False)
        self.lcdNumber.setObjectName("lcdNumber")
        self.gridLayout_4.addWidget(self.lcdNumber, 0, 2, 3, 1)
        self.label_progress_description_2 = QtWidgets.QLabel(self.ControlPanel)
        self.label_progress_description_2.setText("")
        self.label_progress_description_2.setObjectName("label_progress_description_2")
        self.gridLayout_4.addWidget(self.label_progress_description_2, 1, 0, 1, 1)
        self.label_progress_description = QtWidgets.QLabel(self.ControlPanel)
        self.label_progress_description.setObjectName("label_progress_description")
        self.gridLayout_4.addWidget(self.label_progress_description, 0, 0, 1, 1)
        self.pushButton_openDir = QtWidgets.QPushButton(self.ControlPanel)
        self.pushButton_openDir.setMinimumSize(QtCore.QSize(0, 30))
        self.pushButton_openDir.setObjectName("pushButton_openDir")
        self.gridLayout_4.addWidget(self.pushButton_openDir, 0, 1, 2, 1)
        self.gridLayout_4.setColumnStretch(0, 5)
        self.gridLayout_2.addWidget(self.ControlPanel, 4, 0, 1, 2)
        self.MetadataPanel = QtWidgets.QGroupBox(self.centralwidget)
        self.MetadataPanel.setObjectName("MetadataPanel")
        self.gridLayout_11 = QtWidgets.QGridLayout(self.MetadataPanel)
        self.gridLayout_11.setObjectName("gridLayout_11")
        self.gridLayout_3 = QtWidgets.QGridLayout()
        self.gridLayout_3.setObjectName("gridLayout_3")
        self.label_book_name = QtWidgets.QLabel(self.MetadataPanel)
        self.label_book_name.setObjectName("label_book_name")
        self.gridLayout_3.addWidget(self.label_book_name, 0, 0, 1, 1)
        self.lineEdit_title = QtWidgets.QLineEdit(self.MetadataPanel)
        self.lineEdit_title.setObjectName("lineEdit_title")
        self.gridLayout_3.addWidget(self.lineEdit_title, 1, 0, 1, 1)
        self.gridLayout_11.addLayout(self.gridLayout_3, 0, 0, 1, 1)
        self.gridLayout_6 = QtWidgets.QGridLayout()
        self.gridLayout_6.setObjectName("gridLayout_6")
        self.label_autor_name = QtWidgets.QLabel(self.MetadataPanel)
        self.label_autor_name.setObjectName("label_autor_name")
        self.gridLayout_6.addWidget(self.label_autor_name, 0, 0, 1, 1)
        self.lineEdit_artist = QtWidgets.QLineEdit(self.MetadataPanel)
        self.lineEdit_artist.setObjectName("lineEdit_artist")
        self.gridLayout_6.addWidget(self.lineEdit_artist, 1, 0, 1, 1)
        self.gridLayout_11.addLayout(self.gridLayout_6, 1, 0, 1, 1)
        self.gridLayout_7 = QtWidgets.QGridLayout()
        self.gridLayout_7.setObjectName("gridLayout_7")
        self.label_album_name = QtWidgets.QLabel(self.MetadataPanel)
        self.label_album_name.setObjectName("label_album_name")
        self.gridLayout_7.addWidget(self.label_album_name, 0, 0, 1, 1)
        self.lineEdit_album = QtWidgets.QLineEdit(self.MetadataPanel)
        self.lineEdit_album.setObjectName("lineEdit_album")
        self.gridLayout_7.addWidget(self.lineEdit_album, 1, 0, 1, 1)
        self.gridLayout_11.addLayout(self.gridLayout_7, 2, 0, 1, 1)
        self.gridLayout_8 = QtWidgets.QGridLayout()
        self.gridLayout_8.setObjectName("gridLayout_8")
        self.label_album_name_3 = QtWidgets.QLabel(self.MetadataPanel)
        self.label_album_name_3.setObjectName("label_album_name_3")
        self.gridLayout_8.addWidget(self.label_album_name_3, 0, 0, 1, 1)
        self.lineEdit_year = QtWidgets.QLineEdit(self.MetadataPanel)
        self.lineEdit_year.setObjectName("lineEdit_year")
        self.gridLayout_8.addWidget(self.lineEdit_year, 1, 0, 1, 1)
        self.gridLayout_11.addLayout(self.gridLayout_8, 3, 0, 1, 1)
        self.gridLayout_9 = QtWidgets.QGridLayout()
        self.gridLayout_9.setObjectName("gridLayout_9")
        self.label_genre_name = QtWidgets.QLabel(self.MetadataPanel)
        self.label_genre_name.setObjectName("label_genre_name")
        self.gridLayout_9.addWidget(self.label_genre_name, 0, 0, 1, 1)
        self.lineEdit_genre = QtWidgets.QLineEdit(self.MetadataPanel)
        self.lineEdit_genre.setObjectName("lineEdit_genre")
        self.gridLayout_9.addWidget(self.lineEdit_genre, 1, 0, 1, 1)
        self.gridLayout_11.addLayout(self.gridLayout_9, 4, 0, 1, 1)
        self.gridLayout_10 = QtWidgets.QGridLayout()
        self.gridLayout_10.setObjectName("gridLayout_10")
        self.label_albumartist = QtWidgets.QLabel(self.MetadataPanel)
        self.label_albumartist.setObjectName("label_albumartist")
        self.gridLayout_10.addWidget(self.label_albumartist, 0, 0, 1, 1)
        self.lineEdit_albumartist = QtWidgets.QLineEdit(self.MetadataPanel)
        self.lineEdit_albumartist.setObjectName("lineEdit_albumartist")
        self.gridLayout_10.addWidget(self.lineEdit_albumartist, 1, 0, 1, 1)
        self.gridLayout_11.addLayout(self.gridLayout_10, 5, 0, 1, 1)
        self.gridLayout_2.addWidget(self.MetadataPanel, 0, 1, 1, 1)
        self.ConversionSettingsPanel = QtWidgets.QGroupBox(self.centralwidget)
        self.ConversionSettingsPanel.setMaximumSize(QtCore.QSize(16777215, 74))
        self.ConversionSettingsPanel.setObjectName("ConversionSettingsPanel")
        self.verticalLayout_8 = QtWidgets.QVBoxLayout(self.ConversionSettingsPanel)
        self.verticalLayout_8.setObjectName("verticalLayout_8")
        self.verticalLayout_7 = QtWidgets.QVBoxLayout()
        self.verticalLayout_7.setObjectName("verticalLayout_7")
        self.label_audio_quality = QtWidgets.QLabel(self.ConversionSettingsPanel)
        self.label_audio_quality.setObjectName("label_audio_quality")
        self.verticalLayout_7.addWidget(self.label_audio_quality)
        self.comboBox_audio_quality = QtWidgets.QComboBox(self.ConversionSettingsPanel)
        self.comboBox_audio_quality.setObjectName("comboBox_audio_quality")
        self.verticalLayout_7.addWidget(self.comboBox_audio_quality)
        self.verticalLayout_8.addLayout(self.verticalLayout_7)
        self.gridLayout_2.addWidget(self.ConversionSettingsPanel, 2, 1, 1, 1)
        self.groupBox_files = QtWidgets.QGroupBox(self.centralwidget)
        self.groupBox_files.setObjectName("groupBox_files")
        self.verticalLayout = QtWidgets.QVBoxLayout(self.groupBox_files)
        self.verticalLayout.setContentsMargins(-1, 0, -1, 9)
        self.verticalLayout.setObjectName("verticalLayout")
        self.widget = QtWidgets.QWidget(self.groupBox_files)
        self.widget.setMinimumSize(QtCore.QSize(0, 50))
        self.widget.setObjectName("widget")
        self.gridLayout_5 = QtWidgets.QGridLayout(self.widget)
        self.gridLayout_5.setObjectName("gridLayout_5")
        self.pushButton_2 = QtWidgets.QPushButton(self.widget)
        self.pushButton_2.setMinimumSize(QtCore.QSize(0, 30))
        self.pushButton_2.setObjectName("pushButton_2")
        self.gridLayout_5.addWidget(self.pushButton_2, 0, 3, 1, 1)
        self.pushButton = QtWidgets.QPushButton(self.widget)
        self.pushButton.setMinimumSize(QtCore.QSize(0, 30))
        self.pushButton.setObjectName("pushButton")
        self.gridLayout_5.addWidget(self.pushButton, 0, 0, 1, 1)
        self.pushButton_convert = QtWidgets.QPushButton(self.widget)
        self.pushButton_convert.setMinimumSize(QtCore.QSize(0, 30))
        self.pushButton_convert.setObjectName("pushButton_convert")
        self.gridLayout_5.addWidget(self.pushButton_convert, 0, 1, 1, 1)
        self.pushButton_stop_and_clean = QtWidgets.QPushButton(self.widget)
        self.pushButton_stop_and_clean.setMinimumSize(QtCore.QSize(0, 30))
        self.pushButton_stop_and_clean.setObjectName("pushButton_stop_and_clean")
        self.gridLayout_5.addWidget(self.pushButton_stop_and_clean, 0, 4, 1, 1)
        self.verticalLayout.addWidget(self.widget)
        self.tabWidget = QtWidgets.QTabWidget(self.groupBox_files)
        self.tabWidget.setObjectName("tabWidget")
        self.tab_1 = QtWidgets.QWidget()
        self.tab_1.setObjectName("tab_1")
        self.verticalLayout_13 = QtWidgets.QVBoxLayout(self.tab_1)
        self.verticalLayout_13.setContentsMargins(0, -1, 0, -1)
        self.verticalLayout_13.setObjectName("verticalLayout_13")
        self.listWidget = QtWidgets.QListWidget(self.tab_1)
        self.listWidget.setAcceptDrops(True)
        self.listWidget.setToolTip("")
        self.listWidget.setWhatsThis("")
        self.listWidget.setFrameShape(QtWidgets.QFrame.NoFrame)
        self.listWidget.setDragDropMode(QtWidgets.QAbstractItemView.DragDrop)
        self.listWidget.setObjectName("listWidget")
        self.verticalLayout_13.addWidget(self.listWidget)
        self.tabWidget.addTab(self.tab_1, "")
        self.tab_2 = QtWidgets.QWidget()
        self.tab_2.setObjectName("tab_2")
        self.verticalLayout_9 = QtWidgets.QVBoxLayout(self.tab_2)
        self.verticalLayout_9.setObjectName("verticalLayout_9")
        self.tableView = QtWidgets.QTableView(self.tab_2)
        self.tableView.setFrameShape(QtWidgets.QFrame.NoFrame)
        self.tableView.setObjectName("tableView")
        self.verticalLayout_9.addWidget(self.tableView)
        self.tabWidget.addTab(self.tab_2, "")
        self.verticalLayout.addWidget(self.tabWidget)
        self.gridLayout_2.addWidget(self.groupBox_files, 0, 0, 3, 1)
        self.gridLayout = QtWidgets.QGridLayout()
        self.gridLayout.setObjectName("gridLayout")
        self.label_cover_of_book = QtWidgets.QLabel(self.centralwidget)
        self.label_cover_of_book.setMinimumSize(QtCore.QSize(200, 200))
        self.label_cover_of_book.setAlignment(QtCore.Qt.AlignCenter)
        self.label_cover_of_book.setObjectName("label_cover_of_book")
        self.gridLayout.addWidget(self.label_cover_of_book, 1, 0, 1, 1)
        self.pushButton_upload_cover = QtWidgets.QPushButton(self.centralwidget)
        self.pushButton_upload_cover.setObjectName("pushButton_upload_cover")
        self.gridLayout.addWidget(self.pushButton_upload_cover, 0, 0, 1, 1)
        self.gridLayout_2.addLayout(self.gridLayout, 1, 1, 1, 1)
        self.gridLayout_2.setColumnStretch(0, 6)
        self.verticalLayout_12.addLayout(self.gridLayout_2)
        MainWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(MainWindow)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 584, 21))
        self.menubar.setObjectName("menubar")
        MainWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(MainWindow)
        self.statusbar.setObjectName("statusbar")
        MainWindow.setStatusBar(self.statusbar)

        self.retranslateUi(MainWindow)
        self.tabWidget.setCurrentIndex(0)
        QtCore.QMetaObject.connectSlotsByName(MainWindow)

    def retranslateUi(self, MainWindow):
        _translate = QtCore.QCoreApplication.translate
        MainWindow.setWindowTitle(_translate("MainWindow", "MainWindow"))
        self.ControlPanel.setTitle(_translate("MainWindow", "Прогресс выполнения и информация"))
        self.label_progress_description.setText(_translate("MainWindow", "Имя выходного файла"))
        self.pushButton_openDir.setText(_translate("MainWindow", "Открыть папку"))
        self.MetadataPanel.setTitle(_translate("MainWindow", "Панель метаданных"))
        self.label_book_name.setText(_translate("MainWindow", "Название книги"))
        self.label_autor_name.setText(_translate("MainWindow", "Автор"))
        self.label_album_name.setText(_translate("MainWindow", "Альбом"))
        self.label_album_name_3.setText(_translate("MainWindow", "Год"))
        self.label_genre_name.setText(_translate("MainWindow", "Жанр"))
        self.label_albumartist.setText(_translate("MainWindow", "Исполнитель"))
        self.ConversionSettingsPanel.setTitle(_translate("MainWindow", "Панель настроек конвертации"))
        self.label_audio_quality.setText(_translate("MainWindow", "Качество аудио"))
        self.groupBox_files.setTitle(_translate("MainWindow", "Файлы и Кнопки"))
        self.pushButton_2.setText(_translate("MainWindow", "Dell Selected"))
        self.pushButton.setText(_translate("MainWindow", "Add Files"))
        self.pushButton_convert.setText(_translate("MainWindow", "Convert"))
        self.pushButton_stop_and_clean.setText(_translate("MainWindow", "Dell All"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_1), _translate("MainWindow", "Исходные файлы"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_2), _translate("MainWindow", "Результат"))
        self.label_cover_of_book.setText(_translate("MainWindow", "Photo"))
        self.pushButton_upload_cover.setText(_translate("MainWindow", "Загрузить обложку"))


if __name__ == "__main__":
    import sys
    app = QtWidgets.QApplication(sys.argv)
    MainWindow = QtWidgets.QMainWindow()
    ui = Ui_MainWindow()
    ui.setupUi(MainWindow)
    MainWindow.show()
    sys.exit(app.exec_())
