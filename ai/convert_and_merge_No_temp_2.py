...
class M4BMerger:
    ...
    def merge_files(self):
        """Объединяем m4b файлы с помощью ffmpeg, используя stdin для ввода аудиоданных."""
        ffmpeg_input = subprocess.Popen(
            ['ffmpeg', '-f', 'concat', '-safe', '0', '-i', '-', '-c', 'copy', '-y', self.output_file],
        ...
        for file_data in self.input_files:
            # Передаем аудиоданные через stdin в ffmpeg
            ffmpeg_input.stdin.write(file_data.getvalue())

        ffmpeg_input.stdin.close()
        ffmpeg_input.wait()

        if ffmpeg_input.returncode == 0:
